package org.darkquest.client.recorder;
...
public class Recorder implements Runnable, ControllerListener, DataSinkListener {
  ...
  public Recorder(int width, int height, float frameRate,
                  LinkedList<BufferedImage> frames, String output, String format) {
    this.width = width;
    this.height = height;
    this.frames = frames;
  ...
  private boolean waitForConfigure(Processor processor, int limit) {
    synchronized (dummy1) {
      try {
        for (; processor.getState() < limit && configured; dummy1
             .wait())
          ;
      } catch (Exception e) {
      }
  ...
  public void run() {
    try {
      Processor processor = Manager.createProcessor(new ImageSource(
  ...
  private DataSink createSink(Processor processor, MediaLocator locator)
  throws Exception {
