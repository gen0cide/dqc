...
public class GameImage implements ImageProducer, ImageObserver {
  ...
  public GameImage(int width, int height, int k, Component component) {
    ...
    try {
      spriteArchive = new ZipFile(mudclient
                                  .loadCachedFile("Sprites.rscd"));
    } catch (Exception e) {
      e.printStackTrace();
      System.exit(1);
    }
  ...
  public void spriteClip3(int i, int j, int k, int l, int i1, int j1) {
    try {
      ...
      plotScale2(imagePixelArray, sprites[i1].getPixels(), 0, i2, j2, j3,
                 l3, k, l, k2, l2, k1, byte0, j1);
      return;
    } catch (Exception _ex) {
      _ex.printStackTrace();
  ...
  public void spriteClip4(int i, int j, int k, int l, int i1, int overlay,
    ...
    try {
      ...
      if (!flag) {
        spritePlotTransparent(imagePixelArray, sprites[i1].getPixels(),
                              0, k2, l2, j4, k, l, j3, k3, i2, overlay, k1, i3, l3,
                              i5);
        return;
      } else {
        spritePlotTransparent(imagePixelArray, sprites[i1].getPixels(),
                              0, (sprites[i1].getWidth() << 16) - k2 - 1, l2, j4, k,
                              l, -j3, k3, i2, overlay, k1, i3, l3, i5);
        return;
      }
    } catch (Exception _ex) {
      _ex.printStackTrace();
  ...
  private void spritePlotTransparent(int ai[], byte abyte0[], int ai1[],
                                     int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2,
                                     int overlay, int k2, int l2, int i3, int j3) {
    int j4 = overlay >> 16 & 0xff;
    int k4 = overlay >> 8 & 0xff;
    int l4 = overlay & 0xff;
    int i5 = k2 >> 16 & 0xff;
      ...
      for (int offset = 0; offset < string.length(); offset++)
        if (string.charAt(offset) == '@'
          ...
          offset += 4;
                   ...
                   && offset + 5 < string.length()
                   && string.charAt(offset + 5) == '~') {
          char c = string.charAt(offset + 1);
          char c1 = string.charAt(offset + 2);
          char c2 = string.charAt(offset + 3);
